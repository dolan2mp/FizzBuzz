...
describe("A program that implements fizzbuzz", function() {
	...
	it("can take a number and return 'fizz' if it's a multiple of 3, and the number otherwise", function(){
		expect(fb.fizzer(3)).toBe('fizz');
		expect(fb.fizzer(5)).toBe(5);
		expect(fb.fizzer(17)).toBe(17);
	});

	it("can take a number and return 'buzz' if it's a multiple of 5, and the number otherwise", function(){
		expect(fb.buzzer(5)).toBe('buzz');
		expect(fb.buzzer(3)).toBe(3);
		expect(fb.buzzer(17)).toBe(17);
	});

	it("can take a number and return 'fizzbuzz' if it's a multiple of 15, and the number otherwise", function(){
